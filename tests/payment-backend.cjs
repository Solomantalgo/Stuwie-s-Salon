...
// Staff lookup and verification routes use the same ledger rules as sheet reconciliation.
const lookupId=submit(input({paymentReference:'QA-LOOKUP'}));const found=context.findPaymentByReference_({reference:'qa-lookup'});assert.equal(found.ok,true);assert.equal(found.booking.bookingId,lookupId);const staffVerified=context.verifyPaymentFromStaff_({reference:'QA-LOOKUP',amount:'50000'});assert.equal(staffVerified.ok,true);assert.equal(staffVerified.payment.verificationStatus,'Verified');assert.equal(booking(lookupId)[16],50000);const wrongId=submit(input({paymentReference:'QA-WRONG'}));const staffMismatch=context.verifyPaymentFromStaff_({reference:'QA-WRONG',amount:'40000'});assert.equal(staffMismatch.ok,false);assert.equal(staffMismatch.payment.verificationStatus,'Mismatch');assert.equal(booking(wrongId)[16],0);
