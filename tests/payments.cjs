...
(async()=>{
 ...
 assert.equal(await page.locator("#paymentReference").count(),1);await next(page);assert.match(await page.locator("#booking-error").innerText(),/Enter the transaction reference/);await page.locator("#paymentReference").fill("QA-ABC123");
 ...
 assert.equal(record.serviceTotal,10000);assert.equal(record.paymentAmount,5000);assert.equal(record.balanceRemaining,5000);assert.equal(record.paymentStatus,'initiated');assert.ok(record.paymentInitiatedAt);assert.equal(record.paymentVerifiedAt,null);assert.equal(record.paymentReference,'QA-ABC123');assert.match(payloads.at(-1).notes,/Keep these notes/);
 ...
 await select(mobile,'paymentType','full');await mobile.locator('#pay-mobile-money').click();await mobile.locator('#paymentReference').fill('QA-FULL190');await next(mobile);await mobile.getByText('Your request is on its way').waitFor();record=payloads.at(-1).payment;assert.equal(record.paymentAmount,190000);assert.equal(record.balanceRemaining,0);assert.equal(record.paymentStatus,'initiated');assert.ok(record.paymentInitiatedAt);
 const airtel=await setup(true);await visit(airtel);await select(airtel,'paymentProvider','airtel');await next(airtel);assert.match(await airtel.locator('#booking-error').innerText(),/Choose either/);await select(airtel,'paymentType','deposit');await airtel.locator('#pay-mobile-money').click();assert.deepEqual(await airtel.evaluate(()=>window.__handoffs),[]);assert.match(await airtel.locator('#payment-instructions').innerText(),/Airtel Money menu/);await airtel.locator('#paymentReference').fill('QA-AIRTEL');await next(airtel);await airtel.getByText('Your request is on its way').waitFor();assert.equal(payloads.at(-1).payment.paymentProvider,'Airtel Money');assert.equal(payloads.at(-1).payment.paymentStatus,'initiated');
 ...
 const before=payloads.length;await next(unconfigured);assert.match(await unconfigured.locator('#booking-error').innerText(),/Initiate payment before submitting/);assert.equal(payloads.length,before);
 const actual=await browser.newPage({viewport:{width:390,height:844}});const actualErrors=[];actual.on('pageerror',e=>actualErrors.push(e.message));actual.on('console',m=>{if(m.type()==='error')actualErrors.push(m.text())});const actualResponse=await actual.goto('http://127.0.0.1:8080/confirm.html',{waitUntil:'networkidle'});console.log('CONFIRM_HTTP_DOM',JSON.stringify({status:actualResponse.status(),text:(await actual.locator('body').innerText()).slice(0,220),errors:actualErrors,config:await actual.evaluate(()=>Boolean(window.STUWIES_CONFIG)),styles:await actual.evaluate(()=>{const c=getComputedStyle(document.querySelector('main'));return {display:c.display,visibility:c.visibility,opacity:c.opacity,scrollWidth:document.documentElement.scrollWidth,innerWidth}})}));assert.match(await actual.locator('body').innerText(),/Manage Booking/);assert.match(await actual.locator('body').innerText(),/FIND PAYMENT/);assert.deepEqual(actualErrors,[]);const admin=await setup(true);await admin.goto('http://booking.test/confirm.html');console.log('CONFIRM_DOM',JSON.stringify({text:(await admin.locator('body').innerText()).slice(0,300),mainVisible:await admin.locator('main').isVisible(),errors}));assert.match(await admin.locator('body').innerText(),/Manage Booking/);assert.match(await admin.locator('body').innerText(),/FIND PAYMENT/);assert.equal(await admin.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);await admin.goto('http://booking.test/confirm.html?id=TEST');assert.match(await admin.locator('#bookingLabel').innerText(),/Booking TEST/);console.log('CONFIRM_ID_DOM',JSON.stringify({text:(await admin.locator('body').innerText()).slice(0,180),styles:await admin.evaluate(()=>{const c=getComputedStyle(document.querySelector('main'));return {display:c.display,visibility:c.visibility,opacity:c.opacity,width:c.width,scrollWidth:document.documentElement.scrollWidth,innerWidth}})}));assert.deepEqual(errors,[]);await browser.close();console.log('PASS: required choices, deposit/full, recalculation/reset, back/forward, MTN mobile URI, desktop/Airtel fallback, variable prices, missing configuration, unverified records, conflicts, responsive widths. No live requests.');
