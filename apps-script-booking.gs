...
function getStaffSchedule_(params) {
  const requestedDate = normalizeDate_(params && params.date);
  ...
  const bookingsSheet = getSheet_(BOOKINGS_SHEET);
  const paymentsSheet = getSheet_(PAYMENTS_SHEET);
  ...
  paymentRows.slice(1).forEach(function(row) {
    ...
    paymentsByBooking[bookingId].push(row);
  });

  const appointments = [];

  bookingRows.slice(1).forEach(function(row) {
    const bookingId = String(row[0] || '').trim();

    if (!bookingId) return;

    const preferredDate = normalizeDate_(row[9]);

    if (preferredDate !== requestedDate) return;

    const bookingStatus = String(row[12] || '').trim();
    const normalizedStatus = normalizeStatus_(bookingStatus);

    // Do not show cancelled/free/reschedule records in normal schedule.
    if (
    ...
    const service = String(row[6] || '');
    const selectedItems = String(row[7] || '');

    // Exclude obvious product orders from appointment schedule.
    if (
      service.toLowerCase().indexOf('product order') !== -1
    ) {
      return;
    }
    ...
    linkedPayments.forEach(function(paymentRow) {
      if (!latestPayment) {
        latestPayment = paymentRow;
        return;
      }

      const currentCreated = paymentRow[2];
      const latestCreated = latestPayment[2];

      if (
        Object.prototype.toString.call(currentCreated) === '[object Date]' &&
        Object.prototype.toString.call(latestCreated) === '[object Date]' &&
        currentCreated.getTime() > latestCreated.getTime()
      ) {
        latestPayment = paymentRow;
      }
    });
    ...
    const appointment = {
      ...
      paymentStatus:
        String(row[15] || 'Unpaid'),

      amountPaid:
        sheetAmount_(row[16]) || 0,
      ...
      paymentType:
        latestPayment
          ? String(latestPayment[5] || '')
          : '',

      provider:
        latestPayment
          ? String(latestPayment[3] || '')
          : '',

      expectedAmount:
        latestPayment
          ? sheetAmount_(latestPayment[6])
          : null,

      transactionReference:
        latestPayment
          ? String(latestPayment[10] || '')
          : '',

      verificationStatus:
        latestPayment
          ? String(latestPayment[12] || '')
          : ''
    };
